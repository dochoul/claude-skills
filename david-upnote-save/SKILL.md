...
### 5. 원본 링크 노트 정리 안내 (자동 삭제 금지)

⚠️ **DB를 직접 수정해 삭제하지 말 것.** UpNote 클라우드 동기화가 서버 버전을
다시 내려받아 삭제한 노트를 되살린다 (확인된 동작). 읽기는 안전하지만 쓰기/삭제는 불안정하다.

따라서 원본 링크 노트 삭제는 자동화하지 않는다. 대신 사용자에게
"요약이 끝났으니 원본 링크 노트는 UpNote 앱에서 직접 지워주세요"라고 안내한다.

### 6. 완료 알림

읽어온 링크 수와 저장한 요약 목록을 알리고, 원본 링크 노트는 앱에서 직접 삭제하도록 안내한다.
