...
### 5. 연결 (위키의 핵심)
- **허브**(`허브/`, 주제 묶음): 관련 허브 찾기(없으면 생성). 문서 하단 `🔗 **허브**: [[허브명]]`, 허브 "포함 문서"에 `- [[문서stem|제목]]` 추가.
- **엔티티**(`엔티티/`, 인물·기업·도구·핵심개념): 등장 엔티티 찾기(없으면 생성, 1줄 설명+언급문서 목록). 문서 하단 `🏷️ **엔티티**: [[엔티티명]] · ...`, 엔티티 페이지 "언급한 문서"에 `- [[문서stem]]` 추가. **raw 본문에만 나와도** 그 문서의 엔티티로 친다.
- 직접 관련된 다른 문서와도 `[[ ]]`로 상호링크.
- `$WIKI_DIR/index.md`(종합·허브·엔티티·문서 목록) 갱신.
...
3. 여러 문서를 **종합**해 답하고, 근거가 된 문서·엔티티를 `[[ ]]`로 인용한다.
4. **복리(compounding)**: 답이 가치 있으면(반복될 질문·여러 문서 종합이면) `$WIKI_DIR/종합/<주제>.md`로 저장한다(엔티티·문서·허브 링크 포함). 이미 있으면 갱신. index.md에 추가. → 다음엔 다시 만들지 않고 그 페이지를 인용·갱신.

## C. 점검(Lint) — "위키 점검해줘", "건강검진"

`$WIKI_DIR` 전체를 훑어 다음을 점검·보수:
- 어느 허브/엔티티에도 안 묶인 **고아 문서** → 연결
- **끊긴 위키링크**(대상 파일 없는 `[[ ]]`)
- 내용이 **모순**되거나 **오래된** 주장
- 거의 빈 허브·엔티티 / 너무 커진 것(분할 검토)
- `index.md`·`log.md`와 실제 파일 목록 불일치, raw 누락 문서(재수집 가능하면 보강)
발견 사항과 조치를 보고한다.
